...
const MultiSelectFilterContext =
  React.createContext<MultiSelectFilterContextType | null>(null);
...
function useMultiSelectFilter() {
  ...
  if (!context) {
    throw new Error(
      "useMultiSelectFilter must be used within a <MultiSelectFilter />"
    );
  }
...
const MultiSelectFilter = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & MultiSelectFilterProps
...
const MultiSelectCheckbox = ({
  ...
  return (
    <label className={cn("flex items-center gap-2 text-sm", className)}>
      <Checkbox
        checked={isChecked}
        onCheckedChange={(e) => toggleItem(option.id)}
      />
      {option.label}
    </label>
  );
