...
import { FilterOptionType } from "@/entities/constants/job";
...
const FilterSideBar = ({ filterOptions }: Props) => {
  const { filters, updateMultiSelectFilter, updateRangeFilter } = useStore(
    (state) => state.jobs
  );
  return (
    <div className="w-3/12 hidden lg:block sticky h-fit top-24 max-h-[calc(100vh-56px-48px)] overflow-y-auto scrollbar-none shrink-0 bg-white rounded-lg shadow shadow-neutral-200/80 px-4 pt-4 pb-6">
      ...
      <div className="flex flex-col gap-8 mt-6">
        ...
        <FilterWrapper label="Experience range">
          ...
          <div className="flex justify-between">
            <span className="text-xs text-neutral-600">
              {filters.experience[0]} Y
            </span>
            <span className="text-xs text-neutral-600">
              {filters.experience[1]} Y
            </span>
          </div>
        </FilterWrapper>
        ...
        <FilterWrapper label="Work Location">
          <MultiSelectFilter
            ...
            onValueChange={(value) =>
              updateMultiSelectFilter("workLocation", value)
            }
        ...
        <FilterWrapper label="Technology">
          <MultiSelectFilter
            className="flex flex-col gap-1.5"
            onValueChange={(value) =>
              updateMultiSelectFilter("technology", value)
            }
        ...
        <FilterWrapper label="Show">
          <MultiSelectFilter
            className="flex flex-col gap-1.5"
            onValueChange={(value) =>
              updateMultiSelectFilter("showOptions", value)
            }
